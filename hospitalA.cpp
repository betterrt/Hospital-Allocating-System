...
int main(int argc, char *argv[])
{
  ...
  clie_addr.sin_port = htons(30706);
  clie_addr.sin_addr.s_addr = htonl(INADDR_ANY);// set hospital A address because we want hospital A to use fixed port number 30706
  ...
  hospinfo hospA;
  hospA.location = atoi(argv[1]);
  hospA.capacity = atoi(argv[2]);
  hospA.occupancy = atoi(argv[3]);
  sprintf(mname, "%d", hospA.location);

  cout << "Hospital A is up and runnning using UDP on port 30706" << endl;
  cout << "Hospital A has total capacity " << hospA.capacity << " and initial occupancy " << hospA.occupancy << endl;

//loop
 while(1)
 {
   memset(&buf, 0, sizeof(buf));
   sprintf(buf, "A %d %d ", hospA.capacity, hospA.occupancy);
   ...
   cout << "Hospital A has received input from client at location " << startV.name << endl;
   ...
   if(pand == 0)
   {
       cout << "Hospital A does not have the location " << startV.name << " in map" << endl;
   ...
   if(strcmp(startV.name, mname) == 0 || pand == 0)
   {
       score = -1;
       sdistance = -1;
       availability = ((double)hospA.capacity - (double)hospA.occupancy)/(double)hospA.capacity;
       cout << "Hospital A has capacity = "<< hospA.capacity << ", occupation = " << hospA.occupancy << ", availability = "<< availability << endl;
       cout << "Hospital A has the distance = None" << endl;
   ...
   else 
   {
       for(int i = 0; i < VerticesN; i++)
     {
       ...
       {
          score = -1;
          cout << "Hospital A has capacity = "<< hospA.capacity << ", occupation = " << hospA.occupancy << ", availability = None" << endl;
          cout << "Hospital A has found the shortest path to client, distance = " << sdistance << endl;
          cout << "Hospital A has the score = None" << endl;
       }
       else
       {
   ...
   // calculate the score
   memset(&buf, 0, sizeof(buf));
   sprintf(buf, "A %lf %lf", score, sdistance);
   ...
   {
     cout << "Hospital A has sent score = None and distance = " << sdistance << " to the Scheduler" << endl;
   }
   else if(sdistance != -1 && score != -1)
   {
     cout << "Hospital A has sent score = "<< score <<" and distance = " << sdistance << " to the Scheduler" << endl;
   }
   ...
   if(x == 'A')
   {
      hospA.occupancy = hospA.occupancy + 1;
      availability = ((double)hospA.capacity - (double)hospA.occupancy)/(double)hospA.capacity;
      if(availability <= 0)
      {
       cout << "Hospital A has been assigned to a client, occupation is updated to " << hospA.occupancy << ", availability is updated to None" << endl;
      }
      else 
      {
       cout << "Hospital A has been assigned to a client, occupation is updated to " << hospA.occupancy << ", availability is updated to " << availability << endl;  
      }
   }
   cout << "**************************************************************************************" << endl;

...
int main(int argc, char *argv[])
{
  ...
  clie_addr.sin_port = htons(31706);
  clie_addr.sin_addr.s_addr = htonl(INADDR_ANY);// set hospital B address because we want hospital B to use fixed port number 31706
  ...
  hospinfo hospB;
  hospB.location = atoi(argv[1]);
  hospB.capacity = atoi(argv[2]);
  hospB.occupancy = atoi(argv[3]);
  sprintf(mname, "%d", hospB.location);

  cout << "Hospital B is up and runnning using UDP on port 31706" << endl;
  cout << "Hospital B has total capacity " << hospB.capacity << " and initial occupancy " << hospB.occupancy << endl;

//loop
 while(1)
 {
   memset(&buf, 0, sizeof(buf));
   sprintf(buf, "B %d %d ", hospB.capacity, hospB.occupancy);
   ...
   cout << "Hospital B has received input from client at location " << startV.name << endl;
   ...
   if(pand == 0)
   {
       cout << "Hospital B does not have the location " << startV.name << " in map" << endl;
   ...
   if(strcmp(startV.name, mname) == 0 || pand == 0)
   {
       score = -1;
       sdistance = -1;
       availability = ((double)hospB.capacity - (double)hospB.occupancy)/(double)hospB.capacity;
       cout << "Hospital B has capacity = "<< hospB.capacity << ", occupation = " << hospB.occupancy << ", availability = "<< availability << endl;
       cout << "Hospital B has the distance = None" << endl;
   ...
   else 
   {
       for(int i = 0; i < VerticesN; i++)
     {
       ...
       {
          score = -1;
          cout << "Hospital B has capacity = "<< hospB.capacity << ", occupation = " << hospB.occupancy << ", availability = None" << endl;
          cout << "Hospital B has found the shortest path to client, distance = " << sdistance << endl;
          cout << "Hospital B has the score = None" << endl;
       }
       else
       {
   ...
   // calculate the score
   memset(&buf, 0, sizeof(buf));
   sprintf(buf, "B %lf %lf", score, sdistance);
   ...
   {
     cout << "Hospital B has sent score = None and distance = " << sdistance << " to the Scheduler" << endl;
   }
   else if(sdistance != -1 && score != -1)
   {
     cout << "Hospital B has sent score = "<< score <<" and distance = " << sdistance << " to the Scheduler" << endl;
   }
   ...
   if(x == 'B')
   {
      hospB.occupancy = hospB.occupancy + 1;
      availability = ((double)hospB.capacity - (double)hospB.occupancy)/(double)hospB.capacity;
      if(availability <= 0)
      {
       cout << "Hospital B has been assigned to a client, occupation is updated to " << hospB.occupancy << ", availability is updated to None" << endl;
      }
      else 
      {
       cout << "Hospital B has been assigned to a client, occupation is updated to " << hospB.occupancy << ", availability is updated to " << availability << endl;  
      }
   }
   cout << "**************************************************************************************" << endl;
